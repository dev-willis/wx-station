...
const readableTime = (str = Date.now(), seconds = true) => {
...
const dataRefresh = (fullUpdate = true) => {
		...
		.then(data => {
			if(fullUpdate){
				...
				Object.entries(localStorage).forEach(([key, val]) => {
					if(parseInt(key)){
						let dataDate = new Date(parseInt(key)), cutoffDate = new Date(Date.now() - (48 * 60 * 60 * 1000));
